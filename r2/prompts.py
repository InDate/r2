...
# UNIT_TESTER
professional_tester = "you have been provided with this {program_file}, study the file and decide on the best way to test it, provide the approach, then with the following path {unit_test_file} provide the actual tests."
professional_tester_spec_file = "you have been provided with this {spec_file}, provide a unit_test file with the following path {unit_test_file} that provides basic execution tests"
...
# DEBUG
debug = "I am getting this error, please provide a solution - {message}"

# SPEC

spec = """you have been provided with this {file}, provide a specification file with the following path {spec_file_path}, using the format of the example specification below. 
```
- Class Name: Calculator
- Purpose: A simple calculator that can perform addition and subtraction. 

- Properties:
    - `example`: description of properties.

- Methods:
    - `method_1`: Does something.
    - `method_2`: Does something
    - `get_total()`: Returns the current total.

- Example Usage:
example = Example()
example.method(2)
print(example.get_total())  # Outputs: 
```
"""
# COMMANDS
undo_command_reply = (
...
test_connection_message = "Please response with a short joke about being connected to the internet."
